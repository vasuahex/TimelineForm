...
export default function CustomizedTimeline() {
  ...
  const { condition, formData } = useSelector((state: RootState) => state.auth)
          ...
          <TimelineSeparator>
            <div className='flex flex-row justify-center w-full gap-16 cursor-pointer' onClick={() => {
              formData.firstname != "" && formData.lastname != "" && formData.email != "" && formData.mobile != "" && formData.dateofbirth != "" ?
                dispatch(handleCase(2)) : toast.error("Enter All Details!!")
            }}>
              <TimelineConnector sx={timelineStyle} />
              <div>
          ...
          <TimelineSeparator>
            <div className='flex flex-row justify-center w-full gap-16 cursor-pointer' onClick={() => {
              formData.universityname != "" && formData.coursename != "" && formData.fieldofstudy != "" && formData.gpa != "" ?
                dispatch(handleCase(3)) : toast.error("Enter All Details!!")
            }}>
              <div>
